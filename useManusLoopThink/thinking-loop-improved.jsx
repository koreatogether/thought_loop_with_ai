...
export default function ThinkingLoopImproved() {
  const [phase, setPhase] = useState("intro"); // intro | active | complete
  const [seedIdea, setSeedIdea] = useState("");
  const [currentStage, setCurrentStage] = useState(0);
  const [thoughts, setThoughts] = useState([]); // {id, stageId, question, answer, aiCommentary, followUpQuestions, timestamp}
  ...
  const [recommendedNextStages, setRecommendedNextStages] = useState([]);
  ...
  const addWebNode = (thoughtId, stageIdx) => {
    const canvas = canvasRef.current;
    if (!canvas) return;
...
이 단계에 맞는 질문 2~3개와 통찰을 주세요.`;
  ...
  // NEW: 사용자 답변에 대한 AI 코멘트 생성
  const fetchAICommentary = useCallback(
    async (answer, stageIdx, history, selectedQuestion) => {
...
을 JSON 형식으로 주세요.`;
  ...
  // NEW: 추천 다음 단계 계산
  const getRecommendedNextStages = useCallback((currentIdx, allThoughts) => {
    ...
    // 관련 단계 제안 (현재 단계와 색상이 유사한 단계)
    const currentColor = STAGES[currentIdx].color;
  ...
  const handleThoughtNodeClick = (thoughtId) => {
    setSelectedThoughtId(thoughtId);
    setShowThoughtDetail(true);
  };

  const jumpToStage = async (stageIdx) => {
  ...
  return (
    ...
    >
      ...
      {phase === "intro" && (
        ...
        >
          ...
          >
            ...
            <br />
            12단계의 질문 루프가 아이디어를{" "}
            <strong style={{ color: "#f59e0b" }}>살아있는 덩어리</strong>로
            키워냅니다.
            <br />
            <span style={{ fontSize: 12, color: "#555", marginTop: 8 }}>
              ✨ 이제 AI가 당신의 답변에 즉각 반응합니다!
            </span>
          </p>
          <textarea
            value={seedIdea}
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter" && e.metaKey) startLoop();
            }}
            ...
            onFocus={(e) => {
              e.target.style.borderColor = "#f59e0b88";
              e.target.style.boxShadow = "0 0 0 3px #f59e0b22";
            }}
            onBlur={(e) => {
        ...
        >
                          ...
                          {t.followUpQuestions.map((fq, j) => (
                            <div
                              ...
                              style={{
                                ...
                                marginBottom: 4,
                                paddingLeft: 12,
                                borderLeft: `1px solid ${stage.color}44`,
                              }}
                            >
                              • {fq}
                            </div>
                ...
                <>
                    ...
                    {aiQuestions.map((q, i) => (
                      <button
                        ...
                        style={{
                          ...
                          background:
                            selectedQ === i ? `${stage.color}11` : "#111827",
                  ...
                  <div
                    style={{
                      ...
                      padding: 16,
                      border: `1px solid ${
                        selectedQ !== null ? stage.color + "44" : "#1e1e2e"
                      }`,
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button
                          ...
                          style={{
                            ...
                            border: "none",
                            background:
                              currentAnswer.trim() && selectedQ !== null
                                ? `linear-gradient(135deg, ${stage.color}, ${stage.color}aa)`
                                : "#222",
                            color:
                              currentAnswer.trim() && selectedQ !== null
                                ? "#000"
          ...
          >
            ...
            <div style={{ padding: "12px 16px", borderTop: "1px solid #1e1e2e" }}>
              ...
              {STAGES.filter((s) =>
                thoughts.some((t) => t.stageId === s.id)
              ).map((s) => (
                <div
                  key={s.id}
                  style={{
            ...
            >
              ...
              >
                <div
                  style={{
                    fontSize: 12,
                    color: STAGES[selectedThought.stageId - 1]?.color,
                    fontFamily: "JetBrains Mono, monospace",
                    marginBottom: 8,
                  }}
                >
                  {STAGES[selectedThought.stageId - 1]?.icon}{" "}
                  {STAGES[selectedThought.stageId - 1]?.name}
                </div>
                <h2
                  style={{
                    fontSize: 18,
                ...
                {selectedThought.aiCommentary && (
                  <div
                    style={{
                      background: `${STAGES[selectedThought.stageId - 1]?.color}11`,
                      padding: 16,
                      borderRadius: 8,
                      borderLeft: `3px solid ${STAGES[selectedThought.stageId - 1]?.color}`,
                      marginBottom: 16,
                    }}
                  >
                    <div
                ...
                >
                  ...
                  <button
                    onClick={() =>
                      jumpToStage(selectedThought.stageId - 1)
                    }
                    style={{
                      flex: 1,
                      padding: "10px 20px",
      ...
      {phase === "complete" && (
        ...
        >
          <div style={{ fontSize: 48, marginBottom: 8, textAlign: "center" }}>
            ✨
          </div>
          <h1
            style={{
          ...
          {STAGES.map((s) => {
            ...
            return (
              ...
              >
                ...
                >
                  <span>{s.icon}</span>
                  <span style={{ fontWeight: 700, color: s.color, fontSize: 14 }}>
                    {s.name}
                  </span>
                  <span
                    style={{
                ...
                {ts.map((t, i) => (
                  <div
                    ...
                    style={{
                      padding: "14px 20px",
                      borderBottom:
                        i < ts.length - 1 ? "1px solid #1a1a2e" : "none",
                    }}
                  >
